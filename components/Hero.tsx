...
export default function Hero() {
  ...
  const scale = useSpring(
    useTransform(mouseY, [0, 1], [1, 1.04]),
    springConfig
  );

  // 3D depth layers offsets
  const leftLayerZ = useSpring(
    useTransform(mouseY, [0, 1], [20, -20]),
    springConfig
  );
  const rightLayerZ = useSpring(
...
[text-shadow:0_0_20px_rgba(0,0,0,0.1)] text-nowrap
"
          >
            Built for the drop — streetwear heat,
            <br /> luxe essentials, and vintage finds you don’t sleep on.
          </motion.p>
        </motion.div>
      </motion.div>
    </section>
